...
occupancy = (np.histogram(linear_position.values,bins=int(deltaa),range=(mnn,mxx))[0])*(1/1500)
...
countt = np.histogram(spike_pos['speed'],bins=int(deltaaa),range=(mnnn,mxxx))[0]
occupancyy = (np.histogram(speed.values,bins=int(deltaaa),range=(mnnn,mxxx))[0])*(1/1500)
...
hist, xedges, yedges = np.histogram2d(spike_pos['x_pos'], spike_pos['y_pos'], bins=[bin_x,bin_y])
...
hist_vx, xedges, vedges = np.histogram2d(spike_pos['x_pos'], spike_pos['speed'], bins=[bin_x,bin_v])
...
hist_vy, yedges, vedges = np.histogram2d(spike_pos['y_pos'], spike_pos['speed'], bins=[bin_y,bin_v])
...
_,_,mag = scis.spectrogram(x-np.mean(x),fs,window=window,mode='magnitude')
