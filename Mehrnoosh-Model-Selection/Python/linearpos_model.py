...
a_dict = {col_name : linear_position[col_name].values for col_name in linear_position.columns.values}
...
occupancy,bo = np.histogram(spike_position['linear_pos'].values,bins=int(deltaa))
rate = np.divide(count,occupancy)
[spike_binss,pos_bin] = np.histogram(spike_pos['linear_pos'],375,range=(mnn,mxx))
...
for i in np.arange((len(spike_position.index))):
    ...
    p=np.matmul(np.array([u**3,u**2,u,1]),np.array([[-s,2-s,s-2,s],
                [2*s,s-3,3-2*s,-s],[-s,0,s,0],[0,1,0,0]]))
    if nearest_c_pt_index<2:
        nearest_c_pt_index=2
       
      
    concat=np.concatenate((np.zeros(nearest_c_pt_index-2) ,p)) 
    spline.iloc[i] = np.concatenate((concat ,np.zeros(num_c_pts-4-(nearest_c_pt_index-2))))

spline.columns = ['cpt1','cpt2','cpt3','cpt4','cpt5','cpt6','cpt7','cpt8','cpt9','cpt10']
a_dict = {col_name : spline[col_name].values for col_name in spline.columns.values}
