...
lambda_mode0 = np.exp(
    np.sum(np.multiply(beta_mode0, design_matrix.values), axis=1))
...
# Model Fit1
design_matrix_l = design_matrix.assign(
    linear_pos=spike_position['linear_pos'].values)
...
# Model Fit2
design_matrix_ll = design_matrix_l.assign(
    linear_pos_pw2=np.power(spike_position['linear_pos'].values, 2))
...
# Model Fit3
dsgn = design_matrix
mu = np.linspace(np.min(spike_position['linear_pos'].values), np.max(
    spike_position['linear_pos'].values), 10)
variance = 50
...
design_matrix_lrbf = dsgn.assign(
    rbf0=rbf[0], rbf1=rbf[1], rbf2=rbf[2], rbf3=rbf[3], rbf4=rbf[4],
    rbf5=rbf[5], rbf6=rbf[6], rbf7=rbf[7], rbf8=rbf[8], rbf9=rbf[9])
...
# Model fit 4
design_matrixspline = dmatrix(
    'bs(cov, df=8) -1', dict(cov=spike_position['linear_pos'].values))
...
design_matrixspline1 = dmatrix("cc(cov, df=6)", dict(
    cov=spike_position['linear_pos'].values))
...
occupancy = (np.histogram(spike_position['linear_pos'].values, bins=int(
...
for i in np.arange(1, N):
    Z[i] = np.sum(
        lambda_lrbf[spike_pos.index[i - 1]:spike_pos.index[i]].values)
...
for i in np.arange(1, N):
    Z[i] = np.sum(
        lambda_spl1[spike_pos.index[i - 1]:spike_pos.index[i]].values)
...
# Model fit 1
design_matrix_mode1 = design_matrix.assign(
    x_position=spike_position['x_position'].values,
...
lambda_mode1 = np.exp(
    np.sum(np.multiply(beta_mode1, design_matrix_mode1.values), axis=1))
...
for i in np.arange(1, N):
    Z1[i] = np.sum(
        lambda_mode1[spike_pos.index[i - 1]:spike_pos.index[i]].values)
...
lambda_mode2 = np.exp(
    np.sum(np.multiply(beta_mode2, design_matrix_mode2), axis=1))
...
for i in np.arange(1, N):
    Z2[i] = np.sum(
        lambda_mode2[spike_pos.index[i - 1]:spike_pos.index[i]].values)
...
dsgn = design_matrix
xgrid = [64, 65, 65, 65, 69, 72, 80, 85, 86, 88, 93,
         102, 103, 108, 109, 100, 93, 98, 101, 113, 130]
ygrid = [64, 52, 42, 38, 24, 22, 23, 20, 93, 98,
         98, 100, 104, 95, 86, 45, 22, 22, 23, 26, 28]
mu = np.c_[xgrid, ygrid]
variance = 4

data = np.c_[spike_position['x_position'].values,
             spike_position['y_position'].values]
ds = pd.DataFrame({'A': []})
rbf = []
for n in np.arange(len(mu)):
...
design_matrix_rbf = dsgn.assign(rbf0=rbf[0], rbf1=rbf[1], rbf2=rbf[2], rbf3=rbf[3], rbf4=rbf[4],
                                rbf5=rbf[5], rbf6=rbf[6], rbf7=rbf[7], rbf8=rbf[8], rbf9=rbf[
                                    9], rbf10=rbf[10], rbf11=rbf[11], rbf12=rbf[12], rbf13=rbf[13],
...
lambda_rbf = np.exp(
    np.sum(np.multiply(beta_rbf, design_matrix_rbf.values), axis=1))
...
lambda_mode_rbf = np.exp(beta_rbf[0] + beta_rbf[1] * rbf_new[0] + beta_rbf[2] * rbf_new[1] + beta_rbf[3] * rbf_new[2]
                         + beta_rbf[4] * rbf_new[3] + beta_rbf[5] *
                         rbf_new[4] + beta_rbf[6] * rbf_new[5] + beta_rbf[7]
                         * rbf_new[6] + beta_rbf[8] * rbf_new[7] + beta_rbf[9] * rbf_new[8]
                         + beta_rbf[10] * rbf_new[9] + beta_rbf[11] * rbf_new[10] + beta_rbf[12] * rbf_new[11] +
                         beta_rbf[13] * rbf_new[12] + beta_rbf[14] *
                         rbf_new[13] + beta_rbf[15] * rbf_new[14]
                         + beta_rbf[16] * rbf_new[15] + beta_rbf[17] * rbf_new[16] + beta_rbf[18] * rbf_new[17] +
                         beta_rbf[19] * rbf_new[18] + beta_rbf[20] *
                         rbf_new[19] + beta_rbf[21] * rbf_new[20]
                         )

fig = plt.figure(10)
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(xv, yv, lambda_mode_rbf.reshape(
    xv.shape), rstride=10, cstride=10)
...
for i in np.arange(1, N):
    Z1[i] = np.sum(
        lambda_rbf[spike_pos.index[i - 1]:spike_pos.index[i]].values)
